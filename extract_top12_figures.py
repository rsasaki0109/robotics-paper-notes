...
ASSET_DIR = ROOT / "assets" / "figures" / "icra2025-top12"
...
def parse_papers() -> list[dict[str, str]]:
    html = INDEX_HTML.read_text(encoding="utf-8")
    pattern = re.compile(
        r'id: "(p\d+)",.*?title: "([^"]+)".*?arxiv: "(https?://arxiv\.org/abs/([^"]+))"',
        re.S,
    )
    papers = []
    ...
    if not papers:
        raise RuntimeError("Failed to parse papers from index.html")
    return papers
